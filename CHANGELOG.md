...
- **Dart: dropped runtime dependency on `device_info_plus`** — `DeviceOptimizationService.getManufacturerInstructionsUrl` now reads `manufacturer` exclusively from the `getDiagnosticsMetadata` method channel (Android: `Build.MANUFACTURER`; iOS returns `null` since OEM fragmentation doesn't apply). The previous Dart-side fallback to `DeviceInfoPlugin().androidInfo` was unreachable in practice (both paths use a `MethodChannel` and fail together) and led to the same generic dontkillmyapp.com URL on failure. Public API of `DeviceOptimizationService` is unchanged.
