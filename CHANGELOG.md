...
- **Android/iOS: backlog stalls after a transient backend outage until the next explicit `resumeSync()`** — `drainExhaustedContexts` (the per-cycle skip set used by `selectNextLocationBatch` to keep the drain moving past a poisoned batch group) was only cleared inside `resumeSync()`. If a transient outage flipped every active context into the exhausted set and the backend then recovered, the very next batch's 2xx response would clear the persisted auth-failure marker but the drain itself would only complete one batch and then stall — every subsequent `attemptBatchSync` call would log a growing pending count while no further HTTP request fired, because every record's context was still in the exhausted set. Most host apps call `resumeSync()` on cold start or after auth recovery only, so the queue would grow until the next launch. `recordSyncSuccess` now also clears `drainExhaustedContexts` (under the existing `locationSyncLock` on Android), which makes the drain self-healing: any 2xx proves the network path is healthy for *some* context, and that's enough to give the rest a fresh shot on the next drain cycle.
