...
- **Android: plugin builds failed with newer host toolchains (#54)** — The library no longer sets an application-level `targetSdk`, avoiding `LibraryDefaultConfig.setTargetSdk(Integer)` linkage failures when consumed by current Android Gradle Plugin versions.
- **Android: foreground tracking now recovers after eligible process death (#52)** — A recreated service reconciles persisted tracking state and rebuilds the configured notification instead of stopping on a null restart intent or leaving the `Starting...` placeholder visible.
...
- **Dart: dropped runtime dependency on `device_info_plus`** — `DeviceOptimizationService.getManufacturerInstructionsUrl` now reads `Build.MANUFACTURER` via a dedicated, single-purpose `getManufacturer` method channel — Android returns the static `Build.MANUFACTURER` constant (cannot throw); iOS returns `nil` so the Dart caller short-circuits. Previously the value was sourced from `getDiagnosticsMetadata` (which fans out to permission helpers and `systemMonitor.readPowerSaveState()` and could in principle throw) with a Dart-side fallback to `DeviceInfoPlugin().androidInfo`. Both characteristics are gone: the new handler is structurally exception-free, so the helper-throws-but-fallback-saves edge case is impossible by construction, not just by assumption. Public API of `DeviceOptimizationService` is unchanged.
...
- **Android/iOS: Auth-failure pause now persists across process restarts (#35)** — A 401 paused sync in memory only, so the next cold start could retry-storm a stale token. `ConfigManager` now persists the pause reason (`http_401` / `http_403`); `SyncManager` reads it on init, 2xx clears it defensively, and 403 also pauses persistently (it didn't before). Explicit `pause()` stays in-memory only.
